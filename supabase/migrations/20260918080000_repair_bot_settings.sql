...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'bot_settings' and policyname = 'Users can read own bot settings') then
    create policy "Users can read own bot settings" on public.bot_settings for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'bot_settings' and policyname = 'Users can create own bot settings') then
    create policy "Users can create own bot settings" on public.bot_settings for insert with check (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'bot_settings' and policyname = 'Users can update own bot settings') then
    create policy "Users can update own bot settings" on public.bot_settings for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
  end if;
end;
$$;
