  ...
  return (
    <div className="min-h-screen overflow-hidden bg-ink text-zinc-100 grid-bg">
      ...
      <main>
        ...
        <section id="como-funciona" className="relative overflow-hidden py-20"><div className="pointer-events-none absolute left-1/2 top-1/2 h-[580px] w-[900px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/[.055] blur-[150px]" /><div className="relative mx-auto max-w-7xl px-5 lg:px-8"><SectionTitle eyebrow="SIMPLES DO INÍCIO AO FIM" title="Sua automação pronta em 3 passos" text="Sem configurações confusas. Você entra, personaliza e deixa o Adquira Bot trabalhar." /><div className="relative mx-auto mt-10 max-w-6xl"><div className="absolute left-[16.66%] right-[16.66%] top-8 hidden h-px bg-white/[.08] lg:block"><span className="absolute left-0 top-0 h-px w-1/2 bg-gradient-to-r from-primary/20 to-primary/60" /><span className="absolute right-0 top-0 h-px w-1/2 bg-gradient-to-r from-primary/60 to-primary/20" /></div><div className="grid gap-5 lg:grid-cols-3"><HowStep number="01" icon={<MousePointerClick size={22} />} title="Crie seu acesso" text="Cadastre seu nome, defina uma senha segura e entre no seu painel exclusivo." tags={['Cadastro rápido', 'Acesso seguro']} /><HowStep number="02" icon={<KeyRound size={22} />} title="Ative sua licença" text="Insira sua key de acesso e tenha todos os recursos liberados automaticamente." tags={['Ativação imediata', 'Acesso protegido']} /><HowStep number="03" icon={<WandSparkles size={22} />} title="Personalize e inicie" text="Escolha filas, mensagens e presença. Depois é só iniciar e acompanhar tudo." tags={['Configuração visual', 'Controle total']} /></div></div></div></section>
...
function HowStep({ number, icon, title, text, tags }: { number: string; icon: React.ReactNode; title: string; text: string; tags: string[] }) { return <article className="group relative flex h-full flex-col rounded-xl border border-white/[.07] bg-[#0d121b] p-5 transition duration-300 hover:-translate-y-1 hover:border-primary/20"><div className="flex items-center justify-between"><span className="grid h-11 w-11 place-items-center rounded-xl border border-primary/15 bg-primary/[.07] text-primary transition duration-300 group-hover:bg-primary group-hover:text-white">{icon}</span><span className="text-2xl font-extrabold text-white/[.04]">{number}</span></div><h3 className="mt-5 text-base font-bold text-zinc-100">{title}</h3><p className="mt-2 min-h-[48px] text-xs leading-5 text-zinc-500">{text}</p><div className="mt-auto flex flex-wrap gap-1.5 pt-4">{tags.map((tag) => <span key={tag} className="rounded-md border border-white/[.06] bg-white/[.02] px-2 py-1 text-[8px] font-semibold uppercase tracking-wider text-zinc-600">{tag}</span>)}</div></article> }
          ...
          <div className="mt-6 grid items-start gap-6 xl:grid-cols-[1.35fr_.65fr]">
            <section className="overflow-hidden rounded-2xl border border-white/[.08] bg-[#0d131e]">
              ...
              <div className="space-y-5 p-6">
                <div className="grid gap-4 sm:grid-cols-[1fr_auto]"><label className="block"><span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-zinc-500">Canais das filas</span><input value={settings.queue_channel_ids} onChange={(event) => updateSetting('queue_channel_ids', event.target.value.replace(/[^0-9, ]/g, ''))} placeholder="IDs dos canais separados por vírgula" className="w-full rounded-xl border border-white/[.08] bg-[#080d15] px-4 py-3 text-sm outline-none transition placeholder:text-zinc-700 focus:border-primary/60 focus:ring-4 focus:ring-primary/[.07]" /><span className="mt-2 block text-[10px] text-zinc-700">O bot analisará somente estes canais e enviará a mensagem configurada quando detectar uma fila compatível.</span></label><div className="flex items-end"><span className="flex h-[46px] items-center gap-2 rounded-xl border border-amber-400/15 bg-amber-400/[.06] px-4 text-xs font-semibold text-amber-300"><Bot size={15} /> Bot ainda não conectado</span></div></div>
                ...
                <div className="space-y-3"><SettingToggle title="Mencionar jogadores" text="Marca o adversário e os participantes após cada entrada." enabled={settings.mention_players} onChange={(value) => updateSetting('mention_players', value)} /><SettingToggle title="Responder nas filas" text="Envia a mensagem configurada nos canais quando uma nova fila for detectada." enabled={settings.reply_dm} onChange={(value) => updateSetting('reply_dm', value)} /><SettingToggle title="Rich Presence" text="Exibe o status do bot no perfil da conta." enabled={settings.rich_presence} onChange={(value) => updateSetting('rich_presence', value)} /></div>
