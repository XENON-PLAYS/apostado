...
import { Activity as ActivityIcon, ArrowRight, Bell, Bot, CheckCircle2, ChevronRight, Clock3, Eye, EyeOff, Gamepad2, Gauge, KeyRound, LayoutDashboard, LockKeyhole, LogOut, Menu, MessageSquareText, MousePointerClick, Play, Save, Server, Settings2, ShieldCheck, Sparkles, Square, UsersRound, WandSparkles, X, Zap } from 'lucide-react'
      ...
      <header className="fixed inset-x-0 top-0 z-40 border-b border-white/5 bg-ink/75 backdrop-blur-xl">
        <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
          <button onClick={() => navigate('home')} className="flex items-center gap-3 font-bold tracking-tight">
            <img src="./bot-icon.png" alt="Adquira Bot" className="h-10 w-10 rounded-xl object-cover shadow-[0_0_24px_rgba(124,92,252,.35)]" />
  ...
  return (
    <div className="min-h-screen bg-[#070a10] text-zinc-100 selection:bg-primary/30">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[278px] flex-col border-r border-white/[.07] bg-[#0b1019]/95 p-5 backdrop-blur-xl transition-transform duration-300 lg:translate-x-0 ${mobileMenu ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-2 py-2"><div className="flex items-center gap-3"><span className="relative h-11 w-11"><img src="./bot-icon.png" alt="Adquira Bot" className="h-11 w-11 rounded-[14px] object-cover shadow-lg shadow-violet-500/25" /><span className="absolute -right-0.5 -top-0.5 h-3 w-3 rounded-full border-2 border-[#0b1019] bg-emerald-400" /></span><div><p className="font-extrabold tracking-tight">ADQUIRA BOT</p><p className="text-[9px] font-semibold uppercase tracking-[.22em] text-zinc-600">Control center</p></div></div><button onClick={() => setMobileMenu(false)} className="text-zinc-600 lg:hidden"><X size={19} /></button></div>
